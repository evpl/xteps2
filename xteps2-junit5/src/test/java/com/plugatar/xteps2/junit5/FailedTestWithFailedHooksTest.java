...
final class FailedTestWithFailedHooksTest {
  ...
  final RuntimeException expectedException = new CatchExceptionExtension.Exception("test exception");
  final RuntimeException hook2Exception = new RuntimeException("hook2 exception");
  final RuntimeException hook4Exception = new RuntimeException("hook4 exception");
  ...
  void check() {
    ...
    assertThat(testEx)
      ...
      .hasNoCause();
    assertThat(testEx.getSuppressed()).containsExactly(this.hook4Exception, this.hook2Exception);
  }
}
